...
# 2) Terraform
#-------------------------------------------------------------------------------------------------------------------------------------------------#
declare -ag TERRAFORM_MODES=(
  validate
  plan
  )   
  # terraform mode: create | destroy | plan | validate
...
declare -A -g STEPS_EXEC_MODE=(
  ...
  ["01-backups.sh"]="auto"
  ["02-create-spoke-tmpl.sh"]="auto"
  ["03-terraform.sh"]="auto"
  ["04-argo.sh"]="ask"
...
declare -ag DEBUG_MODE_FILES=(
  ...
  "01|$BIN_DIR/settings.sh"
  "02|$BIN_DIR/functions.sh"
  )
