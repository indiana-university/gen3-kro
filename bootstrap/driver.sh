...
: "${LOCAL_WORKING_DIR:=gen3-kro}" # must be set in env file
: "${HUB_TF_DEST:=$LOCAL_WORKING_DIR/terraform/hubs/$ENVIRONMENT}" # must be set in env file
CORE_ISSUE=0
...
fallback_log() {
  ...
  local depth=1
  while [[ "${FUNCNAME[$depth]}" =~ ^(log_info|log_debug|log_warn|log_error|log_notice|log_trace|log_fatal|log|require_env|require_dir|source_or_die)$ ]]; do
    depth=$((depth + 1))
  done
