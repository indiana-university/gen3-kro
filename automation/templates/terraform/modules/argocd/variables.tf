...
variable "vpc_id" {
  description = "VPC ID to be used by pipelines for data"
  type        = string
  default     = ""
}
...
variable "github_app_private_key_ssm_path" {
  ...
  type        = string
  default     = "/gen3-kro-hub/github/private_key"
}
...
variable "argocd_hub_pod_identity_iam_role_arn" {
...
variable "argocd_apps" {
  description = "The list of applications to be managed by ArgoCD"
  type = any
}

variable "addons_metadata" {
  description = "Metadata for the addons"
  type        = map(string)
}

variable "local_addons" {
  description = "Local addons configuration"
  type        = map(any)
}
