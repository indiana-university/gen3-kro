...
# outputs/aws-config-snippet.ini (generated by the prerequisite IAM stack).
#
# Usage:
...
#
# Prerequisites:
#   1. bash scripts/terragrunt-stack.sh prereq-iam apply (generates snippet)
...
#   5. To tear down: bash scripts/terragrunt-stack.sh csoc-core destroy
###############################################################################
set -euo pipefail
...
# That file is generated by: bash scripts/terragrunt-stack.sh prereq-iam apply
...
if [[ -z "$SOURCE_PROFILE" ]]; then
  ...
  echo "  Option 2: run 'bash scripts/terragrunt-stack.sh prereq-iam apply'" >&2
