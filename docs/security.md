...
The `argocd-gitops-bootstrap` Terraform module retrieves this secret at apply time and creates a `kubernetes_secret` in the `argocd` namespace. ArgoCD uses this secret to clone the git repository.

### Kubernetes Secrets

| Secret | Namespace | Contains | Source |
|--------|-----------|----------|--------|
| `{csoc_alias}-csoc-cluster-secret` | `argocd` | Cluster endpoint, CA, metadata | Terraform `argocd-gitops-bootstrap` module |
...
| `outputs/YYYY-MM-DD/` | Operational and Terragrunt logs that may contain resource IDs |
| `outputs/argocd-password.txt` | Current Argo CD bootstrap password, mode `0600` |
...
Set during `helm_release.argocd` via `configs.secret.argocdServerAdminPassword`. The bcrypt hash is auto-generated by ArgoCD. The plaintext is exported as `$ARGOCD_ADMIN_PASSWORD` by `container-init.sh connect` and written to the gitignored `outputs/argocd-password.txt` with mode `0600`.
