...
output "service_roles" {
  ...
  value = {
    for k, v in module.service_role : k => {
      ...
      service_type         = local.services_needing_roles[k].service_type
...
###############################################################################
# Separated ACK and Addon Outputs (for backward compatibility)
###############################################################################
output "ack_all_roles" {
  description = "ACK-only roles (created + override)"
  value = {
    for k, v in local.all_enabled_services : k => {
      role_arn     = v.service_type == "acks" && lookup(v, "override_arn", "") == "" ? module.service_role[k].role_arn : lookup(v, "override_arn", "")
      service_name = k
      spoke_alias  = var.spoke_alias
      source       = lookup(v, "override_arn", "") == "" ? "spoke_created" : "override"
    }
    if v.service_type == "acks"
  }
}

output "addon_all_roles" {
  description = "Addon-only roles (created + override)"
  value = {
    for k, v in local.all_enabled_services : k => {
      role_arn     = v.service_type == "addons" && lookup(v, "override_arn", "") == "" ? module.service_role[k].role_arn : lookup(v, "override_arn", "")
      service_name = k
      spoke_alias  = var.spoke_alias
      source       = lookup(v, "override_arn", "") == "" ? "spoke_created" : "override"
    }
    if v.service_type == "addons"
  }
}
