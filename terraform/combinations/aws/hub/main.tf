...
module "cross_account_policy" {
  ...
  create = var.enable_multi_acct && lookup(each.value, "enable_pod_identity", false) && length(local.spoke_role_arns_by_controller[each.key]) > 0
