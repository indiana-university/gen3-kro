  ...
  csoc_addons_config = {
    for addon_name, addon_config in var.addon_configs : addon_name => merge(
      ...
      lookup(addon_config, "enable_identity", false) ? {
        clientId     = try(module.managed_identities[addon_name].client_id, "")
        principalId  = try(module.managed_identities[addon_name].principal_id, "")
        identityName = try(module.managed_identities[addon_name].identity_name, "")
      } : {},
      {
...
module "csoc_configmap" {
  ...
  cluster_info = {
    ...
    public_subnets            = []
    # Azure specific
    subscription_id = try(data.azurerm_client_config.current.subscription_id, "")
