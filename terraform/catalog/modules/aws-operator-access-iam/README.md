...
2. A **scoped IAM role** (`{csoc_alias}-devcontainer-role`) with least-privilege permissions
...
```bash
cd terraform/catalog/modules/aws-operator-access-iam
...
role_arn = arn:aws:iam::<CSOC_ACCOUNT_ID>:role/<CSOC_ALIAS>-devcontainer-role
...
| Path | Purpose |
| ---- | ------- |
| `terraform/catalog/modules/aws-operator-access-iam/` | Terraform module (IaC) |
