...
module "gitops_argocd_bootstrap" {
  source = "${get_repo_root()}/${values.modules_path}/gitops-argocd-bootstrap"
  ...
  ack_controller_role_arn            = try(data.terraform_remote_state.controller_iam.outputs.ack_controller_role_arn, "")
...
output "git_repository_secret_names" {
  value = module.gitops_argocd_bootstrap.git_repository_secret_names
}

output "argocd_cluster_secret_name" {
  value = module.gitops_argocd_bootstrap.argocd_cluster_secret_name
}

output "bootstrap_applicationset_name" {
  value = module.gitops_argocd_bootstrap.bootstrap_applicationset_name
}
EOF
}
