locals {
  ...
  spoke_role_arns = {
    ...
    alias => "arn:aws:iam::${account_id}:role/${alias}-spoke-role"
  }
  ...
  tags = merge({ Terraform = "true", ManagedBy = "terragrunt", Stack = "fleet" }, lookup(local.config, "tags", {}))
}

unit "spoke_iam" {
  source = "${local.units_path}/spoke-iam"
  path   = "spoke-iam-${local.spoke_alias}"
  ...
  values = {
    ...
    state_key                      = "spokes/${local.spoke_alias}/iam/terraform.tfstate"
    controller_state_key           = "csoc/controller-iam/terraform.tfstate"
    ...
    allow_devcontainer_assume_role = lookup(lookup(local.config, "developer_identity", {}), "allow_devcontainer_assume_role", false)
    roles                          = local.selected_roles
    tags                           = local.tags
  }
}

unit "csoc_spoke_access" {
  source = "${local.units_path}/csoc-spoke-access"
  path   = "csoc-spoke-access"
  ...
  values = {
    ...
    region                  = local.region
    state_key               = "csoc/spoke-access/terraform.tfstate"
    controller_state_key    = "csoc/controller-iam/terraform.tfstate"
    spoke_unit_path         = "../spoke-iam-${local.spoke_alias}"
    ...
    policy_name             = "${local.csoc_alias}-csoc-assume-spokes"
  }
}

unit "argocd_gitops_bootstrap" {
  source = "${local.units_path}/argocd-gitops-bootstrap"
  ...
  values = {
    ...
    profile                    = local.profile
    region                     = local.region
    state_key                  = "csoc/gitops-bootstrap/terraform.tfstate"
    cluster_state_key          = "csoc/cluster/terraform.tfstate"
    controller_state_key       = "csoc/controller-iam/terraform.tfstate"
    argocd_install_state_key   = "csoc/argocd-install/terraform.tfstate"
    spoke_access_unit_path     = "../csoc-spoke-access"
    spoke_role_arns            = local.spoke_role_arns
