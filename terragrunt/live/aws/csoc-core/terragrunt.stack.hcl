locals {
  ...
  tags            = merge({ Terraform = "true", ManagedBy = "terragrunt", Stack = "csoc-core" }, lookup(local.config, "tags", {}))
...
unit "csoc_cluster" {
  source = "${local.units_path}/csoc-cluster"
  path   = "csoc-cluster"

  values = merge(local.common, {
    state_key                                = "csoc/cluster/terraform.tfstate"
...
unit "csoc_controller_iam" {
  source = "${local.units_path}/csoc-controller-iam"
  path   = "csoc-controller-iam"

  values = merge(local.common, {
    state_key                        = "csoc/controller-iam/terraform.tfstate"
    cluster_unit_path                = "../csoc-cluster"
...
unit "argocd_install" {
  source = "${local.units_path}/argocd-install"
  path   = "argocd-install"

  values = merge(local.common, {
    state_key                  = "csoc/argocd-install/terraform.tfstate"
    cluster_unit_path          = "../csoc-cluster"
    controller_unit_path       = "../csoc-controller-iam"
